...
// To count number of words from: http://stackoverflow.com/questions/12698836/counting-words-in-a-string-c-programming
	...
	//write(STDOUT_FILENO, "Here3", 5);
	//printf("\nfd_in = %d \n\n", fd_in);
/*
	i = 0;
	if (strchr(proc2,32)) {
		write(STDOUT_FILENO, "Haha1\n", 6);
		char* tokens1 = strtok(proc2," |\0");
		while (tokens1 != NULL) {
			head2[i] = tokens1;
			tokens1 = strtok(NULL," |\0");
			i++;
	...
	if ((pid = fork()) == -1)
      	...
      	else if (pid == 0)
        {
			...
			if(j == k){
				dup2(STDOUT_FILENO, pd[1]);
			}
			if(j < k){
				dup2(STDOUT_FILENO, pd[0]);
			}
			close(pd[0]);
      	...
      	else
        {
			...
			//}
			close(pd[1]);
			fd_in = pd[0]; //save the input for the next command
	...
	/*
        ...
        {
			//if(j>1)
				//dup2( STDIN_FILENO, fd_in); //change the input according to the old one 
			//else if(j==k)
...
void piping_test(char* cmd) {
	int pcount=1; //already one pipe exist
	char *head=NULL, *tail=NULL, *head_p=NULL;
	char *arg[100]={NULL};
	int ofd=0, fd[2];
	pid_t pid;
	int i = 0;
...
void redirection(char* cmd, char *red1, char *red2) {
	...
	if(red1!=NULL) { // '>'
		...
		if(pid == -1) {
			...
			//source: http://stackoverflow.com/questions/15798450/open-with-o-creat-was-it-opened-or-created
			fd=open(redt, O_CREAT | O_RDWR | S_IROTH, 0644);
			if(fd == -1) {
...
void execute_command(char* cmd, int input_count) {
	...
	//Source: http://www.cplusplus.com/reference/cstring/strchr/
	pch = strchr(cmd,124);
...
void builtin_command(char *token, int input_count) {
	if(strcmp(token,"exit") == 0){
		...
		char* current_dir = get_current_dir_name(); //Source: https://linux.die.net/man/3/getcwd
		char* token = strtok(NULL,"\0");
		//write(2,"\n",1);
		if ( input_count == 2) { //user types only cd, then we go to home directory
			char* username = getlogin(); //Source: http://pubs.opengroup.org/onlinepubs/009695399/functions/getlogin.html
		...
		//parse_input(arr);
			...
			if (input_char == 0) { //there's nothing for input
				...
				if (history_count<10) { //there are up to 9 history
					memcpy(history[history_count], &arr, sizeof(arr));
					history_input_char[history_count]++;
					history_count++;
					...
					for (int i=0;i<9;i++) {
						...
						history_input_char[i] = history_input_char[i+1];
					}
					//add the user input to the newest history
					memcpy(history[9],arr,input_char+1);
					history_input_char[9] = input_char;
				}
			...
			if (c == 0x5B) { //second part of arrow
				...
				if (c == 0x41) { //up-arrow
					...
					if (history_count>0 && history_current>0) {
						...
						//input_char = strlen(history[history_current]); //gives warning
						//input_char = history_input_char[history_current]; //pls get rid of warning

						input_char = strlen(history[history_current]); //doesn't work fully
						//printf("sizeof(history[history_current][0] is: %d\n",sizeof(history[history_current][0]));
						//printf("history[current] is: %s and has size of: %d",history[history_current],input_char);
					}
				}
					...
					if (history_count<11) {
						...
						if (history_current<history_count) { //make sure we don't go past last history
							...
							//input_char = strlen(history[history_current]); //gives warning
							//input_char = sizeof(history[history_current])/sizeof(history[history_current][0]);
							//input_char = history_input_char[history_current]; //pls get rid of warning

							//printf("sizeof(history[history_current][0] is: %d\n",sizeof(history[history_current][0]));
							//printf("history[current] is: %s and has size of: %d\n",history[history_current],input_char);
						}
					}
				}
			}
